package busbike.base.process.exception;
...
import busbike.base.process.result.ExceptionResultInfo;
import busbike.base.process.result.ResultInfo;
...
public class ExceptionResolverCustom implements HandlerExceptionResolver {
	...
	public ModelAndView resolveException(HttpServletRequest request,
		...
		// 输出 异常信息
		ex.printStackTrace();
		// 转成springmvc底层对象（就是对action方法的封装对象，只有�?个方法）
		...
		modelAndView.addObject("exceptionResultInfo",
				exceptionResultInfo.getResultInfo());
		modelAndView.setViewName(view);// 逻辑视图�?
		return modelAndView;
	}
