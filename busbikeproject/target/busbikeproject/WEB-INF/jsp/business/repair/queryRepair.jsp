...
function repairJob(repairId){
	var sendUrl = "${baseurl}repair/repairJob.action?repairId="+repairId;
	createmodalwindow("维修工作提交", 900, 500, sendUrl);
}
var gysypmladd = function (){
	//alert(id);
	var sendUrl = "${baseurl}repair/addmaintain.action";
...
var columns = [ [{
	...
	width : 60,
	formatter:function(value, row, index){
		return '<a href=javascript:repairJob(\''+row.repairId+'\')>提交</a>';
	}
}]];
...
function initGrid(){
	$('#gysypmllist').datagrid({
		title : '维修工作表',
		striped : true,
		url : '${baseurl}repair/queryRepair_result.action',
...
<div id="ypxxquery_div">
	<%-- <form id="ypxxlistFrom" name="gysypmldeleteForm" action="${baseurl}repair/exportMaintainSubmit.action" method="post">
	</form>  --%>
    <form id="gysypmldeleteForm" name="gysypmldeleteForm" action="${baseurl}repair/deletegysypmlsubmit.action" method="post">
			...
			<TABLE  class="table_search">
				<TBODY>
					<TR>
						...
						<TD class="left">序号：</td>
						<td><INPUT type="text"  name="repairInfoCustom.repairId" /></TD>
						<!-- 自行添加 -->
						<TD class="left">报修用户名：</TD>
						<td ><INPUT type="text" name="repairInfoCustom.reportName" /></td>
						<!-- 自行添加 -->
						<TD class="left">服务点编号：</TD>
						<td ><INPUT type="text" name="repairInfoCustom.rentNetId" /></td>
						<!-- 自行添加 -->
					...
					<TR>
						...
						<td ><INPUT type="text" name="repairInfoCustom.reportDate" /></td>
						<!-- 自行添加 -->
						 <td class="left">维修日期：</td>
				  		<td>
				      		<INPUT id="repairInfoCustom.repairDate" name="repairInfoCustom.repairDate" style="width:70px"/>
							至
							<INPUT id="repairInfoCustom.repairDate" name="repairInfoCustom.repairDate" style="width:70px"/>
					...
					<tr>
					   <!-- 自行添加 -->
						...
						<td >
							<select id="ypxxCustom_lb" name="repairInfoCustom.state" style="width:150px">	
						...
						<td >
							<select id="ypxxCustom.jyzt" name="repairInfoCustom.repairMode" style="width:150px">
							     <option value="">全部</option>
				  		...
				  		<td>
				  		<select id="ypxxCustom.zlcc" name="repairInfoCustom.isMultiFault" style="width:150px">
								<option value="">全部</option>
				  		...
				  		<td>
				  		<select id="gysypmlCustom.control" name="repairInfoCustom.faultType" style="width:80px">
								<option value="">全部</option>
